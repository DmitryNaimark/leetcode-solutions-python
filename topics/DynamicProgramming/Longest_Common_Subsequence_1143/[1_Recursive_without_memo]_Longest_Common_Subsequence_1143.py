...
# Runtime Complexity: O(2^(text1 + text2))
...
class Solution:
    ...
    def longestCommonSubsequence(self, text1: str, text2: str) -> int:
        self.longest_subseq = 0
        self.lcs_recursively(text1, text2, 0, 0, 0)

        return self.longest_subseq

    def lcs_recursively(self, s1, s2, i, j, subseq):
        if i == len(s1) or j == len(s2):
            self.longest_subseq = max(subseq, self.longest_subseq)
            return

        if s1[i] == s2[j]:
            self.lcs_recursively(s1, s2, i + 1, j + 1, subseq + 1)
        else:
            self.lcs_recursively(s1, s2, i + 1, j, subseq)
            self.lcs_recursively(s1, s2, i, j + 1, subseq)
