...
class Solution:
    def splitArray(self, nums: List[int]) -> bool:
        ...
        for j in range(3, n - 3):
            ...
            # Skip recalculating zeros
            if j > 3 and nums[j] == 0 == nums[j - 1]:
                continue

            for i in range(1, j - 1):
                if get_sum_inclusive(0, i - 1) == get_sum_inclusive(i + 1, j - 1):
                    set_sums.add(get_sum_inclusive(0, i - 1))

            if len(set_sums) > 0:
                for k in range(j + 2, n - 1):
                    sum_last = get_sum_inclusive(k + 1, n - 1)
                    if get_sum_inclusive(j + 1, k - 1) == sum_last and sum_last in set_sums:
                        return True
