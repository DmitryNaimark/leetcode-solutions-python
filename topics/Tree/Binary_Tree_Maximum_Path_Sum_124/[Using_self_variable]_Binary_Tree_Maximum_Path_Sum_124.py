...
class Solution:
    ...
    def maxPathSum(self, root: TreeNode) -> int:
        # Have to reinitialize it, because I init Solution object only once.
        self.total_sum = -2 ** 63

        self.max_path_sum_recursively(root)
        return self.total_sum

    def max_path_sum_recursively(self, node: TreeNode) -> int:
        if not node:
            return 0

        left_sum = max(self.max_path_sum_recursively(node.left), 0)
        right_sum = max(self.max_path_sum_recursively(node.right), 0)

        self.total_sum = max(left_sum + node.val + right_sum, self.total_sum)
